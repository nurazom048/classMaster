import 'dart:convert';
import 'package:classmate/features/routine_fetures/presentation/utils/popup.dart';
...
class RoutineRequestImpl implements RoutineRepositoryImp {
  ...
  Future<Either<Message, Message>> createRoutine({
    ...
    print(
      '🚀 [HTTP POST /routine] Body: name="$routineName", routineType="$routineType"',
    );

    try {
      final response = await http.post(
        uri,
        body: jsonEncode({"name": routineName, "routineType": routineType}),
        headers: headers,
      );

      print(
        '📥 [HTTP POST /routine] Response status=${response.statusCode}, body=${response.body}',
      );
      final res = json.decode(response.body);
  ...
  Future<AllClassesResponse> getAllClasses(String routineID) async {
    ...
    try {
      if (!isOnline && isHaveCache) {
        ...
        final res = json.decode(response.body);
        print(
          '📥 [HTTP GET /routine/$routineID/classes] Response routineType=${res['routineType']}',
        );
        if (response.statusCode == 200) {
  ...
  Future<Either<String, CheckStatusModel>> classNotification({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final statusModel = CheckStatusModel.fromJson(
