...
import 'package:classmate/features/routine_fetures/data/models/weekday_list_models.dart';
...
class WeekDayControllerClass extends StateNotifier<AsyncValue<WeekdayList>> {
  ...
  void getStatus() async {
    if (!mounted) return;
    try {
      final WeekdayList data = await routineRepository.getAllWeekdaysInClass(
        classId,
      );
  ...
  Future<void> deleteWeekday(
    BuildContext context,
    String id,
    String classId,
  ) async {
    Either<Message, WeekdayList> res = await routineRepository
        .deleteWeekdayById(id, classID: classId);
