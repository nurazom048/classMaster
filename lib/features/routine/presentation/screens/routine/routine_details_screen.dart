...
class _RoutineDetailsScreenState extends ConsumerState<RoutineDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    if (widget.data != null) {
      return DesktopLayoutWrapper(
        child: _buildScaffoldWithData(widget.data!),
      );
    }
  ...
  Widget _buildScaffoldWithData(AllClassesResponse data) {
    final checkStatus = ref.watch(checkStatusControllerProvider(widget.routineId));
    final statusData = checkStatus.value;
    final isExam = data.routineType.toUpperCase() == "EXAM";
    final canModify = widget.isOwnerOrCaptain ??
        (statusData?.isOwner == true || statusData?.isCaptain == true);

    final String appBarTitle = widget.routineName.isNotEmpty
        ? widget.routineName
                      ...
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) => SummaryScreen(
                            classId: data.allClass.isNotEmpty
